...
def check_eleven(card: list, total: int):
    """This deals with the functionality of ace which can be 11 or 1 depending on the situation."""
    if 11 in card and total > 21:
        card[-1] = 1
        total -= 10

cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10] * 16
...
while True:
    ...
    for i in player_cards:
        player_total += i
        check_eleven(player_cards, player_total)
    ...
    if player_total != 21:
        ...
        for i in comp_cards:
            comp_total += i
            check_eleven(comp_cards, comp_total)
        if comp_total == 21:
            print('It is a tie. You get your money back.')
        else:
